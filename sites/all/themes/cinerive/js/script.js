...
//				if(video match youtube)
		...
		// CSS - EN
		$('#block-views-shows-eng-blockschowsen h2').after("<div class='border'> </div>");
		...
		// Add color to "horaires et billets" in ENGLISH
		if( $('#block-views-shows-eng-blockschowsen').length ){
			$('#block-views-shows-eng-blockschowsen .view-grouping').each(function(){
				var town = $(this).find('.view-grouping-header').html().toLowerCase();
				$(this).addClass(town);
			})
			// replace days name in french for ENGLISH
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Lundi/g,'Monday');
			});
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Mardi/g,'Tuesday');
			});
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Mercredi/g,'Wednesday');
			});	
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Jeudi/g,'Thursday');
			});	
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Vendredi/g,'Friday');
			});	
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Samedi/g,'Saturday');
			});
			$('#block-views-shows-eng-blockschowsen').html(function(index,html){
			    return html.replace(/Dimanche/g,'Sunday');
			});
